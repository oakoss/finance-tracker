...
export const Route = createFileRoute('/_app/budgets')({
  ...
  loader: async ({ context, deps }) => {
    const periods = await context.queryClient.ensureQueryData(
      budgetPeriodQueries.list(),
    );
...
function BudgetsPage() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 px-4 py-6 lg:px-6">
      <BudgetsPageHeader periods={periods} />
      {periods.length === 0 ? (
        <BudgetsEmpty variant="noPeriods" />
      ) : (activePeriod ? (
        <BudgetOverview periodId={activePeriod.id} />
      ) : (
        <BudgetsEmpty variant="noCurrentPeriod" />
      ))}
    </div>
  );
}
