...
- Better Auth schema uses UUIDs generated by the DB; a postprocess step (`scripts/postprocess-auth-schema.ts`) replaces `gen_random_uuid()` with `uuidv7()`, normalizes timestamps to `timestamptz`, removes explicit column names, and strips generated relations (managed in module relation files instead).
...
- Negative: Requires a postprocess step after Better Auth schema generation (also handles timestamp normalization, column name cleanup, and relation stripping).
