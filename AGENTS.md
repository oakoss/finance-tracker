...
- Do not edit `src/modules/auth/db/schema.ts` — generated by Better Auth CLI, then postprocessed (`pnpm schema:auth`). The postprocess step normalizes timestamps, removes explicit column names, strips generated relations, and runs ESLint + Prettier.
...
- Generated schema: `src/modules/auth/db/schema.ts` (do not edit manually).
...
- **auth** — `api/` (server functions, e.g., `get-session.ts`), `db/` (generated schema — do not edit, relations), `emails/` (React Email templates, rendering service, sending service), `hooks/` (e.g., `use-sign-out.ts`), `middleware.ts`
- **finance** — `db/` (schema, relations) — accounts, transactions, categories, payees, imports, debt strategies, promotions, recurring rules, etc.
- **todos** — `db/` (schema, relations) — minimal example module

Each module owns its Drizzle schema and relations in `{module}/db/`. All are re-exported through `src/db/schema.ts` (the aggregator).

**Relations pattern:** Each module defines its own relations in `src/modules/{module}/db/relations.ts`. Drizzle requires exactly one `relations()` call per table, so `usersRelations` lives in `src/modules/auth/db/relations.ts` since `users` is an auth table — it consolidates relations spanning auth, finance, and todos modules.
...
Auth hooks in `src/modules/auth/hooks/`:
